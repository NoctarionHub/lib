...
do
	...
	local previousUnload = globalTable.__NHUI_Unload
	globalTable.__NHUI_Unload = nil
...
NHUI.Config = { Blur = true, MaxNotifications = 5 }
...
local function RefreshAcrylicEffect()
	...
	dof.Enabled = NHUI.Config.Blur ~= false and #AcrylicControllers > 0
end
...
local function CreateWindowAcrylic(guiObject)
	...
	folder.Name = "NHUI_AcrylicWindow"
	...
	function controller:Update()
		...
		local visible = NHUI.Config.Blur ~= false
...
function NHUI:SetBlurEnabled(enabled)
	NHUI.Config.Blur = enabled and true or false
	RefreshAcrylicEffect()
	UpdateAllAcrylic()
end
...
local ASSETS_FOLDER = "NHUI/Assets"
...
local function EnsureAssetsFolder()
	...
	local ok = pcall(function()
		if not fn_isfolder("NHUI") then fn_makefolder("NHUI") end
...
local function PrivateTabFlagPath(name)
	...
	return "NHUI/PrivateTab_" .. safe .. ".remember"
end
...
local RunCountPath = "NHUI/RunCount.txt"
...
local function AddContentScrollThumb(scroll, listLayout, thumbParent, janitor)
	...
	thumb.BackgroundColor3 = NHUI.Theme.TextDim
...
local function MeasureText(text, size, maxWidth)
	...
	local ok, bounds = pcall(function()
		return TextService:GetTextSize(
			text, size, NHUI.Theme.MeasureFont,
...
function NHUI:GetIcon(name, source)
...
local function ResolveIcon(icon)
	...
	if source and name then
		return NHUI:GetIcon(name, source)
	end
	return NHUI:GetIcon(icon, "Lucide")
end
 
function NHUI:PreloadIcons(sources)
...
local function GetRoot()
	...
	local stale = parent:FindFirstChild("NHUI")
	if stale then stale:Destroy() end
...
function NHUI:SetScaleRange(minScale, maxScale)
...
function NHUI:Notify(opts)
	...
	card.LayoutOrder = NHUI._NotifyCounter
	card.ZIndex = Z.Toast
	card.Visible = false
	card.Parent = holder
	Corner(card, 12)
	...
	if text ~= "" then
		...
		textLabel.FontFace = NHUI.Theme.FontRegular
		textLabel.Text = text
		textLabel.TextColor3 = NHUI.Theme.TextDim
...
function NHUI:Confirm(opts)
	...
	dialog.BackgroundColor3 = NHUI.Theme.Surface
	...
	titleLabel.FontFace = NHUI.Theme.Font
	titleLabel.Text = title
	titleLabel.TextColor3 = danger and NHUI.Theme.Danger or NHUI.Theme.Text
	titleLabel.TextTransparency = 1
...
function NHUI:Modal(opts)
	...
	for i, field in ipairs(fields) do
		...
		if labelH > 0 then
			local lbl = Instance.new("TextLabel")
			lbl.BackgroundTransparency = 1
			lbl.FontFace = NHUI.Theme.FontRegular
			lbl.Text = string.upper(field.Label)
			lbl.TextColor3 = danger and NHUI.Theme.Danger or NHUI.Theme.TextDim
			lbl.TextSize = 11
		...
		box.FontFace = NHUI.Theme.FontRegular
		...
		box.TextColor3 = NHUI.Theme.Text
		box.TextSize = 13
		...
		jan:Add(box.Focused:Connect(function()
			Tween(fieldStroke, { Color = NHUI.Theme.Accent, Transparency = 0.3 }, 0.15)
...
local function AddEmptyState(scroll, overlayParent, janitor)
	...
	icon.ImageColor3 = NHUI.Theme.TextDim
	...
	label.FontFace = NHUI.Theme.FontRegular
	label.Text = "There's nothing here yet"
	label.TextColor3 = NHUI.Theme.TextDim
	label.TextSize = 13
...
function NHUI:CreateWindow(opts)
	...
	GlassLayer(main, NHUI.Theme.CornerRadius, 0.985)
	...
	local function addControl(icon, name, order, hoverColor)
		...
		ic.ImageColor3 = NHUI.Theme.TextDim
		...
		jan:Add(btn.MouseEnter:Connect(function()
			...
			Tween(ic, { ImageColor3 = hoverColor or NHUI.Theme.Text }, 0.12)
		end))
		jan:Add(btn.MouseLeave:Connect(function()
			Tween(btn, { BackgroundTransparency = 1 }, 0.12)
			Tween(ic, { ImageColor3 = NHUI.Theme.TextDim }, 0.12)
		end))
		return btn
	end
	...
	if hasIcon then
		...
		windowIcon.ImageColor3 = NHUI.Theme.Text
	...
	resizeHandle.ImageColor3 = NHUI.Theme.TextDim
...
function Window:Destroy()
	...
	local idx = table.find(NHUI._Windows, self)
	if idx then table.remove(NHUI._Windows, idx) end
...
function Window:AddDockButton(opts)
	...
	jan:Add(btn.MouseEnter:Connect(function()
		...
		Tween(icon, { ImageColor3 = NHUI.Theme.Text }, 0.12)
	end))
	jan:Add(btn.MouseLeave:Connect(function()
		if active then return end
		Tween(btn, { BackgroundTransparency = 0.95 }, 0.12)
		Tween(icon, { ImageColor3 = NHUI.Theme.TextDim }, 0.12)
	end))
	...
	return {
		...
		SetActive = function(_, isActive)
			...
			Tween(icon, { ImageColor3 = active and NHUI.Theme.Text or NHUI.Theme.TextDim }, 0.12)
		end,
	}
end
...
function Window:AddDefaultCreditsPanel()
	...
	titleIcon.ImageColor3 = NHUI.Theme.Text
	...
	closeIcon.ImageColor3 = NHUI.Theme.TextDim
	...
	closeBtn.MouseEnter:Connect(function() closeIcon.ImageColor3 = NHUI.Theme.Text end)
	closeBtn.MouseLeave:Connect(function() closeIcon.ImageColor3 = NHUI.Theme.TextDim end)
	...
	for i, credit in ipairs(CREDITS) do
		...
		nameLabel.FontFace = NHUI.Theme.Font
		nameLabel.Text = credit.Name
		nameLabel.TextColor3 = NHUI.Theme.Text
		...
		roleLabel.FontFace = NHUI.Theme.FontRegular
		roleLabel.Text = credit.Role
		roleLabel.TextColor3 = NHUI.Theme.TextDim
...
card.BackgroundTransparency = 0.96
	...
	statusLabel.FontFace = NHUI.Theme.FontRegular
	statusLabel.Text = "Bridge disconnected"
	statusLabel.TextColor3 = NHUI.Theme.TextDim
	...
	nowPlayingTag.FontFace = NHUI.Theme.Font
	...
	trackLabel.FontFace = NHUI.Theme.Font
	trackLabel.Text = "Nothing playing"
	trackLabel.TextColor3 = NHUI.Theme.Text
	...
	artistLabel.FontFace = NHUI.Theme.FontRegular
	artistLabel.Text = "Connect your Spotify bridge"
	artistLabel.TextColor3 = NHUI.Theme.TextDim
	...
	seekBubbleLabel.FontFace = NHUI.Theme.Font
	seekBubbleLabel.Text = "0:00"
	seekBubbleLabel.TextColor3 = NHUI.Theme.Text
	...
	timeLabel.FontFace = NHUI.Theme.FontRegular
	timeLabel.Text = "0:00"
	timeLabel.TextColor3 = NHUI.Theme.TextDim
	...
	durationLabel.FontFace = NHUI.Theme.FontRegular
	durationLabel.Text = "0:00"
	durationLabel.TextColor3 = NHUI.Theme.TextDim
...
button.BackgroundTransparency = primary and 0.05 or 0.94
		...
		image.ImageColor3 = primary and Color3.fromRGB(12, 28, 18) or NHUI.Theme.TextDim
	...
	local function makeInputCard(order, title, placeholder, buttonIcon)
		...
		label.Text = title
		label.TextColor3 = NHUI.Theme.Text
		label.TextSize = 12
...
button.BackgroundTransparency = 0.91
		...
		buttonImage.ImageColor3 = NHUI.Theme.Text
	...
	favoritesTitle.FontFace = NHUI.Theme.Font
	favoritesTitle.Text = "Favorite playlists"
	favoritesTitle.TextColor3 = NHUI.Theme.Text
	...
	favoritesHint.FontFace = NHUI.Theme.FontRegular
	favoritesHint.Text = "Save and load your playlists with one tap"
	favoritesHint.TextColor3 = NHUI.Theme.TextDim
	...
	favoritesSearchIcon.ImageColor3 = NHUI.Theme.TextDim
	...
	favoritesSearchBox.FontFace = NHUI.Theme.FontRegular
	...
	favoritesSearchBox.TextColor3 = NHUI.Theme.Text
	...
	addFavoriteIcon.ImageColor3 = NHUI.Theme.Text
	...
	saveFavoriteTitle.FontFace = NHUI.Theme.Font
	saveFavoriteTitle.Text = "Save current playlist"
	saveFavoriteTitle.TextColor3 = NHUI.Theme.Text
	...
	saveFavoriteHint.FontFace = NHUI.Theme.FontRegular
	saveFavoriteHint.Text = "Add the playlist loaded in the player to Favorites"
	saveFavoriteHint.TextColor3 = NHUI.Theme.TextDim
	...
	saveFavoriteChevron.ImageColor3 = NHUI.Theme.TextDim
	...
	local function renderFavorites()
		...
		for visibleIndex = 1, count do
			...
			playlistIcon.ImageColor3 = NHUI.Theme.Text
			...
			name.FontFace = NHUI.Theme.Font
			name.Text = tostring(favorite.name or "Spotify playlist")
			name.TextColor3 = NHUI.Theme.Text
			name.TextSize = 11
			...
			subtitle.FontFace = NHUI.Theme.FontRegular
			subtitle.Text = "Spotify playlist"
			subtitle.TextColor3 = NHUI.Theme.TextDim
			subtitle.TextSize = 9
			...
			loadFavoriteIcon.ImageColor3 = NHUI.Theme.Text
	...
	local function paintModes()
		...
		controlRefs.Shuffle.Icon.ImageColor3 = shuffle and Color3.fromRGB(30, 215, 96) or NHUI.Theme.TextDim
		controlRefs.Repeat.Icon.ImageColor3 = repeatMode ~= "off" and Color3.fromRGB(30, 215, 96) or NHUI.Theme.TextDim
	end
	...
	connectBridge = function()
		...
		task.spawn(function()
			...
			send("hello", { client = "NHUI", protocol = 1 })
		end)
	end
	...
	return {
		...
		},
		{
			...
			Handler = function(args)
				local ok, err = NHUI:SetUIElementValue(args.flag, args.value)
...
function Window:_BuildDefaultSystemPrompt()
	...
	return "You are a helpful assistant embedded in a Roblox UI panel built with NHUI. Your tools "
...
function Window:AddChatPanel(opts)
	...
	local function headerIconButton(icon, layoutOrder)
		...
		jan:Add(btn.MouseEnter:Connect(function()
			Tween(btn, { BackgroundTransparency = 0.9 }, 0.12)
			Tween(ic, { ImageColor3 = NHUI.Theme.Text }, 0.12)
		end))
		jan:Add(btn.MouseLeave:Connect(function()
			Tween(btn, { BackgroundTransparency = 1 }, 0.12)
			Tween(ic, { ImageColor3 = NHUI.Theme.TextDim }, 0.12)
...
pill.BackgroundTransparency = 0.95 
	...
	sendIcon.ImageColor3 = NHUI.Theme.Text
	...
	local function addBubble(text, role)
		...
		local avatar = Instance.new("Frame")
		avatar.Name = "Avatar"
		avatar.BackgroundColor3 = isUser and Color3.new(1, 1, 1) or NHUI.Theme.Accent
		...
		if isUser then
			...
			botIcon.ImageColor3 = NHUI.Theme.Accent
		...
		bubble.Name = "Bubble"
		bubble.BackgroundColor3 = isUser and NHUI.Theme.Accent or Color3.new(1, 1, 1)
		...
		for i, seg in ipairs(segments) do
			if seg.kind == "code" then
				local card = Instance.new("Frame")
				card.Name = "CodeBlock"
				card.BackgroundColor3 = NHUI.Theme.Background
				...
				langLabel.FontFace = NHUI.Theme.FontRegular
				langLabel.Text = seg.lang
				langLabel.TextColor3 = NHUI.Theme.TextDim
				...
				jan:Add(copyBtn.MouseButton1Click:Connect(function()
					...
					task.delay(0.4, function()
						if copyIcon.Parent then
							Tween(copyIcon, { ImageColor3 = NHUI.Theme.TextDim }, 0.15)
						end
					end)
				end))
				...
				codeLabel.TextColor3 = NHUI.Theme.Text
	...
	local function addToolLine(name)
		...
		toolIcon.ImageColor3 = NHUI.Theme.Accent
	...
	local function buildTypingRow()
		...
		local avatar = Instance.new("Frame")
		avatar.BackgroundColor3 = NHUI.Theme.Accent
		avatar.BackgroundTransparency = 1
		avatar.BorderSizePixel = 0
		avatar.Size = UDim2.fromOffset(AVATAR, AVATAR)
		...
		for i = 1, 3 do
			...
			dot.BackgroundColor3 = NHUI.Theme.TextDim
...
function Window:AddCloudPanel(opts)
			...
			NHUI:Modal({
				...
				Callback = function(confirmed, values)
					...
					if not values.Name or values.Name:gsub("%s+", "") == "" then
						NHUI:Notify({ Title = "Local Save", Text = "Name can't be empty.", Type = "warning", Duration = 3 })
						return
					end
					local function saveNow()
						local ok, err = NHUI:SaveConfig(values.Name, { Description = values.Description })
						NHUI:Notify({
					...
					if NHUI:GetConfigMeta(values.Name) then
						NHUI:Confirm({
	...
	localGrid = CloudTabs.Local:AddCardGrid({
		...
		Fetch = function(state)
			local items, err = NHUI:ListConfigs()
			...
			for _, cfg in ipairs(items) do
				...
				local function loadLocalConfig()
					NHUI:Confirm({
						...
						Callback = function(confirmedLoad)
							...
							local snapshot = NHUI:CreateSnapshot()
							local ok, loadErr = NHUI:LoadConfig(cfg.Name, false)
							if not ok then
								NHUI:Notify({ Title = "Could not load", Text = tostring(loadErr), Type = "error", Duration = 4 })
								return
							end
							NHUI:Notify({
								Title = "Loaded", Text = tostring(cfg.Name) .. " is now active.", Type = "success", Duration = 6,
								Actions = {{ Text = "Undo", Callback = function() NHUI:RestoreSnapshot(snapshot, false) end }},
							})
						end,
					})
				end
				...
				local function renameLocalConfig()
					NHUI:Modal({
						...
						Callback = function(confirmed, values)
							...
							local existing = NHUI:GetConfigMeta(newName)
							if existing and newName ~= cfg.Name then
								NHUI:Notify({ Title = "Name already used", Text = "Choose another config name.", Type = "warning", Duration = 3 })
								return
							end
							local ok, renameErr = NHUI:RenameConfig(cfg.Name, newName)
							NHUI:Notify({ Title = ok and "Renamed" or "Could not rename", Text = ok and "Config name updated." or tostring(renameErr), Type = ok and "success" or "error", Duration = 3 })
							if ok and localGrid then localGrid.Refresh() end
						end,
					})
				end
				...
				local function publishLocalConfig()
					if not service then
						NHUI:Notify({ Title = "Cloud", Text = "No cloud service configured.", Type = "warning", Duration = 3 })
						return
					end
					local saved, savedErr = NHUI:GetSavedConfig(cfg.Name)
					if not saved then
						NHUI:Notify({ Title = "Could not read config", Text = tostring(savedErr), Type = "error", Duration = 3 })
						return
					end
					local result, publishErr = service:Publish({ Name = cfg.Name, Description = cfg.Description, Tags = cfg.Tags }, saved.Data)
					NHUI:Notify({ Title = result and "Published" or "Could not publish", Text = result and "Config published successfully." or tostring(publishErr), Type = result and "success" or "error", Duration = 4 })
					if result then
				...
				local function deleteLocalConfig()
					NHUI:Confirm({
						...
						Callback = function(confirmed)
							if not confirmed then return end
							local ok, deleteErr = NHUI:DeleteConfig(cfg.Name)
							NHUI:Notify({ Title = ok and "Deleted" or "Could not delete", Text = ok and "Config removed." or tostring(deleteErr), Type = ok and "success" or "error", Duration = 3 })
							if ok and localGrid then localGrid.Refresh() end
						end,
					})
				end
				...
				local function deletePublishedConfig()
					NHUI:Confirm({
						...
						Callback = function(confirmedDelete)
							...
							NHUI:Notify({ Title = ok and "Deleted" or "Could not delete", Text = ok and "Config removed." or tostring(delErr), Type = ok and "success" or "error", Duration = 3 })
							if ok then
								if mineGrid then mineGrid.Refresh() end
								if publicGrid then publicGrid.Refresh() end
							end
					...
					Callback = function()
						NHUI:Confirm({
							...
							Callback = function(confirmedApply)
								...
								NHUI:SetConfig(result.Data, false)
								NHUI:Notify({
									Title = "Applied",
									...
									Actions = {
										{
											Text = "Undo",
											Callback = function()
												NHUI:RestoreSnapshot(snapshot, false)
												NHUI:Notify({
													Title = "Reverted",
...
pill.BackgroundTransparency = 0.95 
	...
	local function addBubble(msg, isOwn)
		...
		avatar.BackgroundColor3 = isAnon and Color3.fromRGB(196, 143, 105) or NHUI.Theme.Accent
		avatar.BackgroundTransparency = 1
		avatar.BorderSizePixel = 0
		avatar.Size = UDim2.fromOffset(AVATAR, AVATAR)
		...
		local bubble = Instance.new("Frame")
		bubble.Name = "Bubble"
		bubble.BackgroundColor3 = isOwn and NHUI.Theme.Accent or Color3.new(1, 1, 1)
		bubble.BackgroundTransparency = 1
		bubble.BorderSizePixel = 0
		...
		if msg.CreatedAt then
			...
			timeLbl.FontFace = NHUI.Theme.FontRegular
			timeLbl.Text = os.date("%H:%M", math.floor(msg.CreatedAt / 1000))
			timeLbl.TextColor3 = isOwn and Color3.new(1, 1, 1) or NHUI.Theme.TextDim
		...
		if not isOwn and msg.Id and service then
			...
			reportIcon.ImageColor3 = NHUI.Theme.TextDim
			...
			jan:Add(reportBtn.MouseEnter:Connect(function()
				...
				Tween(reportIcon, { ImageColor3 = NHUI.Theme.Danger, ImageTransparency = 0 }, 0.1)
			end))
			jan:Add(reportBtn.MouseLeave:Connect(function()
				Tween(reportBtn, { BackgroundTransparency = 1 }, 0.1)
				Tween(reportIcon, { ImageColor3 = NHUI.Theme.TextDim }, 0.1)
			end))
	...
	local function trySend()
		...
		task.spawn(function()
			...
			if not result then
				NHUI:Notify({ Title = "Chat", Text = tostring(err), Type = "error", Duration = 3 })
				return
			end
	...
	local function openSettingsPopup()
		...
		popup.BackgroundColor3 = NHUI.Theme.Surface
		...
		local function toggleRow(order, label, getValue, onToggle)
			...
			checkIcon.ImageColor3 = NHUI.Theme.Accent
...
function Window:AddTab(nameOrOpts)
	...
	local glowColor = opts.GlowColor or NHUI.Theme.Accent
	...
	if isPrivate and not remembered then
		...
		lockBadge.BackgroundColor3 = NHUI.Theme.Background
	...
	if hasIcon then
		...
		iconLabel.ImageColor3 = NHUI.Theme.TextDim
	...
	textLabel.BackgroundTransparency = 1
	textLabel.FontFace = NHUI.Theme.Font
	textLabel.Text = name
	textLabel.TextColor3 = NHUI.Theme.TextDim
	textLabel.TextSize = 14
	textLabel.TextXAlignment = Enum.TextXAlignment.Left
	...
	local function selectTab()
		...
		for _, t in pairs(self._tabs) do
			...
			Tween(t._label, { TextColor3 = isSelected and NHUI.Theme.Text or NHUI.Theme.TextDim }, 0.22)
			if t._icon then
				Tween(t._icon, { ImageColor3 = isSelected and NHUI.Theme.Text or NHUI.Theme.TextDim }, 0.22)
			end
...
function Window:_PromptTabPassword(tabObj, callback)
	...
	dialog.BackgroundColor3 = NHUI.Theme.Background
	dialog.BackgroundTransparency = 1
	dialog.BorderSizePixel = 0
	dialog.ClipsDescendants = true
	dialog.AutomaticSize = Enum.AutomaticSize.Y
	...
	lockIcon.BackgroundTransparency = 1
	lockIcon.Image = ResolveIcon("lock")
	lockIcon.ImageColor3 = NHUI.Theme.TextDim
	...
	title.FontFace = NHUI.Theme.Font
	title.Text = "Private Tab"
	title.TextColor3 = NHUI.Theme.Text
	title.TextSize = 16
	...
	maskLabel.FontFace = NHUI.Theme.FontRegular
	maskLabel.Text = ""
	maskLabel.TextColor3 = NHUI.Theme.Text
	...
	placeholderLabel.FontFace = NHUI.Theme.FontRegular
	placeholderLabel.Text = "Password"
	placeholderLabel.TextColor3 = NHUI.Theme.TextDim
	...
	rememberLabel.FontFace = NHUI.Theme.FontRegular
	rememberLabel.Text = "Remember me"
	rememberLabel.TextColor3 = NHUI.Theme.TextDim
	...
	local function shakeError()
		Tween(fieldStroke, { Color = NHUI.Theme.Danger, Transparency = 0.4 }, 0.12)
		placeholderLabel.Text = "Incorrect password"
		Tween(placeholderLabel, { TextColor3 = NHUI.Theme.Danger }, 0.12)
		...
		task.delay(1.4, function()
			...
			Tween(placeholderLabel, { TextColor3 = NHUI.Theme.TextDim }, 0.3)
...
function Window:_JumpToSearchable(entry)
	...
	task.delay(0.6, function()
		...
		inst.BackgroundColor3 = NHUI.Theme.Accent
...
function Window:_OpenSearch()
	...
	panel.BackgroundColor3 = NHUI.Theme.Background
	...
	closeBtn.MouseEnter:Connect(function()
		...
		Tween(closeIcon, { ImageColor3 = NHUI.Theme.Text }, 0.12)
	end)
	closeBtn.MouseLeave:Connect(function()
		Tween(closeBtn, { BackgroundTransparency = 1 }, 0.12)
		Tween(closeIcon, { ImageColor3 = NHUI.Theme.TextDim }, 0.12)
	end)
	...
	box.Text = ""
	box.TextColor3 = NHUI.Theme.Text
	box.PlaceholderColor3 = NHUI.Theme.TextDim
	box.TextSize = 15
	box.ClearTextOnFocus = false
	box.TextXAlignment = Enum.TextXAlignment.Left
	...
	local function rebuild(query)
		...
		for i, entry in ipairs(matches) do
			...
			titleLbl.FontFace = NHUI.Theme.Font
			titleLbl.Text = entry.title
			titleLbl.TextColor3 = NHUI.Theme.Text
			...
			pathLbl.FontFace = NHUI.Theme.FontRegular
			pathLbl.Text = SearchEntryPath(entry.tabObj)
			pathLbl.TextColor3 = NHUI.Theme.TextDim
			pathLbl.TextSize = 11
...
function Tab:AddSubTab(nameOrOpts)
	...
	if not self._subTabHolder then
		...
		self._subTabScrollTrack.BackgroundColor3 = NHUI.Theme.TextDim
		...
		self._subTabScrollThumb.BackgroundColor3 = NHUI.Theme.TextDim
	...
	lbl.FontFace = NHUI.Theme.Font
	lbl.Text = title
	lbl.TextColor3 = NHUI.Theme.TextDim
	lbl.TextSize = 13
...
function Tab:SelectSubTab(idx)
	...
	for i, st in pairs(self._subTabs) do
		...
		Tween(st.Label, { TextColor3 = sel and NHUI.Theme.Text or NHUI.Theme.TextDim }, 0.22)
		if st.Icon then
			Tween(st.Icon, { ImageColor3 = sel and NHUI.Theme.Text or NHUI.Theme.TextDim }, 0.22)
		end
	end
...
function Tab:AddParagraph(opts)
	...
	if opts.Title then
		...
		if iconAsset ~= "" then
			...
			img.BackgroundTransparency = 1
			img.Image = iconAsset
			img.ImageColor3 = NHUI.Theme.Text
...
local function BuildStarRow(parent, layoutOrder, maxStars, starColor, starSize, default)
	...
	local function paint(previewCount)
		...
		for i, button in ipairs(stars) do
			...
			Tween(button, { ImageColor3 = on and starColor or NHUI.Theme.TextDim }, 0.12)
		end
	end
	...
	for i = 1, maxStars do
		...
		button.ImageColor3 = NHUI.Theme.TextDim
...
function NHUI:SanitizeText(text, opts)
	opts = opts or {}
...
function NHUI:SendFeedbackWebhook(webhookUrl, stars, message, opts)
	opts = opts or {}
...
function Tab:AddRating(opts)
	...
	feedback.SendBtn.MouseButton1Click:Connect(function()
		...
		if opts.WebhookUrl then
			task.spawn(function()
				NHUI:SendFeedbackWebhook(opts.WebhookUrl, selected, feedback.Box.Text, opts.WebhookOptions)
			end)
		end
...
function Tab:AddButton(opts)
	...
	chev.ImageColor3 = NHUI.Theme.TextDim
	...
	click.MouseEnter:Connect(function()
		...
		Tween(chev, { ImageColor3 = NHUI.Theme.Text, Position = UDim2.new(1, -12, 0.5, 0) }, 0.15)
	end)
	click.MouseLeave:Connect(function()
		Tween(card, { BackgroundTransparency = 0.96 }, 0.15)
		Tween(chev, { ImageColor3 = NHUI.Theme.TextDim, Position = UDim2.new(1, -16, 0.5, 0) }, 0.15)
	end)
	click.MouseButton1Click:Connect(function()
		Tween(card, { BackgroundTransparency = 0.8 }, 0.08)
		Tween(chev, { ImageColor3 = NHUI.Theme.Accent }, 0.08)
		task.delay(0.08, function()
			if not card.Parent then return end
			Tween(card, { BackgroundTransparency = 0.9 }, 0.15)
			Tween(chev, { ImageColor3 = NHUI.Theme.Text }, 0.15)
		end)
		if opts.Callback then task.spawn(opts.Callback) end
	end)
...
function Tab:AddCard(opts)
	...
	if hasImage then
		...
		Corner(imgHolder, NHUI.Theme.CornerRadiusSm)
		...
		Corner(img, NHUI.Theme.CornerRadiusSm)
	...
	if hasButton then
		...
		footerLabel.FontFace = NHUI.Theme.Font
		footerLabel.Text = opts.ButtonText
		footerLabel.TextColor3 = NHUI.Theme.Text
	...
	if hasRating then
		...
		if ratingHasTitle then
			...
			ratingLabel.FontFace = NHUI.Theme.Font
			ratingLabel.Text = ratingOpts.Title
			ratingLabel.TextColor3 = NHUI.Theme.Text
		...
		feedback.SendBtn.MouseButton1Click:Connect(function()
			...
			if ratingOpts.WebhookUrl then
				task.spawn(function()
					NHUI:SendFeedbackWebhook(ratingOpts.WebhookUrl, sel, feedback.Box.Text, ratingOpts.WebhookOptions)
				end)
			end
			if clearOnSubmit then
				feedback.Box.Text = ""
...
function Tab:AddChangelogEntry(opts)
	...
	versionLabel.FontFace = NHUI.Theme.Font
	versionLabel.Text = version
	versionLabel.TextColor3 = NHUI.Theme.Text
	...
	if date then
		...
		dateLabel.FontFace = NHUI.Theme.FontRegular
		dateLabel.Text = date
		dateLabel.TextColor3 = NHUI.Theme.TextDim
	...
	for i, change in ipairs(changes) do
		...
		pillLabel.FontFace = NHUI.Theme.Font
		...
		changeLabel.FontFace = NHUI.Theme.FontRegular
		changeLabel.Text = tostring(change.Text or "")
		changeLabel.TextColor3 = NHUI.Theme.TextDim
...
function Tab:AddLoadoutGroup(opts)
	...
	local color = opts.Color or NHUI.Theme.Accent
	...
	titleLabel.TextColor3 = NHUI.Theme.TextDim
...
slot.BackgroundTransparency = 0.95
		...
		Corner(slot, NHUI.Theme.CornerRadiusSm)
...
function Tab:AddInfoGrid(opts)
	...
	if rows > 0 then
		...
		for i, item in ipairs(items) do
			...
			labelLabel.TextColor3 = NHUI.Theme.Text
	...
	if hasFooter then
		...
		btnRow.Position = UDim2.fromOffset(0, height - PAD - BUTTON_GAP_BOTTOM - BUTTON_H)
...
function Tab:AddLeaderboard(opts)
	...
	container.BackgroundColor3 = NHUI.Theme.Surface
	...
	Corner(container, NHUI.Theme.CornerRadiusSm)
	...
	revealIcon.ImageColor3 = NHUI.Theme.TextDim
	...
	jan:Add(revealBtn.MouseEnter:Connect(function()
		...
		Tween(revealIcon, { ImageColor3 = NHUI.Theme.Text }, 0.12)
	end))
	jan:Add(revealBtn.MouseLeave:Connect(function()
		Tween(revealBtn, { BackgroundTransparency = 1 }, 0.12)
		Tween(revealIcon, { ImageColor3 = NHUI.Theme.TextDim }, 0.12)
	end))
	...
	local function buildRow(index, item)
		...
		row.ZIndex = Z.Content + 2
		row.Parent = list
		Corner(row, NHUI.Theme.CornerRadiusSm)
		...
		if rankColor then
			...
			badgeLabel.FontFace = NHUI.Theme.Font
			badgeLabel.Text = "#" .. tostring(index)
			badgeLabel.TextColor3 = NHUI.Theme.TextDim
		...
		if item.UserId and item.UserId ~= 0 then
			...
			placeholder.ImageColor3 = NHUI.Theme.TextDim
...
function Tab:AddGradientCard(opts)
	...
	if opts.Callback then
		...
		veil.Parent = card
		Corner(veil, NHUI.Theme.CornerRadiusSm)
...
pill.BackgroundTransparency = 0.95 
	...
	penIcon.ImageColor3 = NHUI.Theme.TextDim
...
pill.BackgroundTransparency = 0.95 
	...
	keyIcon.ImageColor3 = NHUI.Theme.TextDim
...
function Tab:AddConsole(opts)
	...
	titleIcon.ImageColor3 = NHUI.Theme.TextDim
	titleIcon.Size = UDim2.fromOffset(14, 14)
	titleIcon.LayoutOrder = 1
	titleIcon.ZIndex = Z.Content + 3
	titleIcon.Parent = titleRow
	...
	emptyIcon.ImageColor3 = NHUI.Theme.TextDim
	...
	local function addLog(message, messageType)
		...
		counter = counter + 1
		local color = ConsoleColors[messageType] or NHUI.Theme.Text
 
		local entry = Instance.new("TextLabel")
		entry.Name = "Entry"
		entry.BackgroundTransparency = 1
		entry.RichText = true
		entry.FontFace = NHUI.Theme.FontRegular
		entry.TextSize = 12
		...
		entry.Text = string.format(
			...
			NHUI.Theme.TextDim:ToHex(), os.date("%H:%M:%S"),
	...
	for ci, col in ipairs(columns) do
		...
		if sortable then
			cellBtn.MouseEnter:Connect(function()
				if sortState.Key ~= col.Key then Tween(lbl, { TextColor3 = NHUI.Theme.Text }, 0.12) end
			end)
			cellBtn.MouseLeave:Connect(function()
				if sortState.Key ~= col.Key then Tween(lbl, { TextColor3 = NHUI.Theme.TextDim }, 0.12) end
			end)
		end
	end
	...
	local function renderRows()
		...
		for ri, row in ipairs(currentRows) do
			...
			for ci, col in ipairs(columns) do
				...
				cell.FontFace = NHUI.Theme.FontRegular
				cell.Text = tostring(row[col.Key] == nil and "" or row[col.Key])
				cell.TextColor3 = NHUI.Theme.Text
				cell.TextSize = 12
			...
			if cellBtn then
				cellBtn.MouseButton1Click:Connect(function()
					...
					for key, info in pairs(headerLabels) do
						...
						info.Lbl.TextColor3 = (key == sortState.Key) and NHUI.Theme.Text or NHUI.Theme.TextDim
					end
					applySort()
				end)
			end
...
function Tab:AddCardGrid(opts)
	...
	Corner(outer, NHUI.Theme.CornerRadiusSm)
...
searchPill.BackgroundTransparency = 0.92
		...
		searchBox.FontFace = NHUI.Theme.FontRegular
		...
		searchBox.Text = ""
		searchBox.TextColor3 = NHUI.Theme.Text
		searchBox.TextSize = 13
	...
	if #sorts > 1 then
		...
		for i, sortName in ipairs(sorts) do
			...
			lbl.TextColor3 = (sortName == currentSort) and NHUI.Theme.Text or NHUI.Theme.TextDim
			lbl.TextSize = 11
			lbl.AutomaticSize = Enum.AutomaticSize.X
	...
	gridScroll.ScrollBarImageColor3 = NHUI.Theme.TextDim
	...
	statusIcon.ImageColor3 = NHUI.Theme.TextDim
	...
	local function openCardMenu(anchor, actions)
		...
		for i, action in ipairs(actions) do
			...
			icon.ImageColor3 = action.Danger and NHUI.Theme.Danger or NHUI.Theme.TextDim
			icon.Size = UDim2.fromOffset(14, 14)
			...
			label.TextColor3 = action.Danger and NHUI.Theme.Danger or NHUI.Theme.Text
			label.TextSize = 12
			label.TextXAlignment = Enum.TextXAlignment.Left
	...
	local function buildCard(item, animDelay)
		...
		Corner(cell, NHUI.Theme.CornerRadiusSm)
		...
		if item.Icon then
			...
			iconImg.ImageColor3 = NHUI.Theme.Accent
		...
		if hasPrimaryAction then
			...
			actionBadge.BackgroundColor3 = NHUI.Theme.Accent
			...
			actionIcon.ImageColor3 = NHUI.Theme.Accent
		...
		if hasMenuAction then
			...
			menuBadge.BackgroundColor3 = NHUI.Theme.Accent
			...
			menuIcon.ImageColor3 = NHUI.Theme.Accent
		...
		if item.Description and item.Description ~= "" then
			...
			descLbl.FontFace = NHUI.Theme.FontRegular
			descLbl.Text = item.Description
			descLbl.TextColor3 = NHUI.Theme.TextDim
			descLbl.TextSize = 11
		...
		if item.Byline and item.Byline ~= "" then
			...
			bylineLbl.FontFace = NHUI.Theme.FontRegular
			bylineLbl.Text = item.Byline
			bylineLbl.TextColor3 = NHUI.Theme.TextDim
		...
		if item.Stats and #item.Stats > 0 then
			...
			for i, stat in ipairs(item.Stats) do
				...
				statIcon.ImageColor3 = NHUI.Theme.TextDim
				...
				statLbl.FontFace = NHUI.Theme.FontRegular
				statLbl.Text = tostring(stat.Text or "")
				statLbl.TextColor3 = NHUI.Theme.TextDim
				statLbl.TextSize = 10
				...
				if stat.Callback then
					statFrame.MouseEnter:Connect(function()
						Tween(statIcon, { ImageColor3 = NHUI.Theme.Accent }, 0.1)
						Tween(statLbl, { TextColor3 = NHUI.Theme.Accent }, 0.1)
					end)
					statFrame.MouseLeave:Connect(function()
						Tween(statIcon, { ImageColor3 = NHUI.Theme.TextDim }, 0.1)
						Tween(statLbl, { TextColor3 = NHUI.Theme.TextDim }, 0.1)
					end)
	...
	for sortName, entry in pairs(sortButtons) do
		entry.Button.MouseButton1Click:Connect(function()
			...
			for otherName, otherEntry in pairs(sortButtons) do
				...
				Tween(otherEntry.Label, { TextColor3 = active and NHUI.Theme.Text or NHUI.Theme.TextDim }, 0.12)
			end
			refresh()
		end)
	end
...
function NHUI:GetConfig()
	local data = {}
	for flag, api in pairs(NHUI.Flags) do
		if api.Get then
...
function NHUI:SetConfig(data, silent)
	if type(data) ~= "table" then return false end
	for flag, raw in pairs(data) do
		local api = NHUI.Flags[flag]
...
function NHUI:SetUIElementValue(flag, value, silent)
	local api = NHUI.Flags[flag]
...
local CONFIGS_FOLDER = "NHUI/Configs"
...
local function LegacyConfigPath(name)
	return "NHUI/" .. SafeConfigName(name) .. ".json"
end
...
function NHUI:SaveConfig(name, opts)
	if not fn_writefile then return false, "writefile unavailable" end
	opts = opts or {}
	EnsureConfigsFolder()
	name = name or "config"
	local envelope = BuildConfigEnvelope(name, NHUI:GetConfig(), opts)
	local ok, err = pcall(function()
...
function NHUI:LoadConfig(name, silent)
	name = name or "config"
	...
	return NHUI:SetConfig(envelope.Data, silent)
end
 
function NHUI:GetConfigMeta(name)
	local envelope, err = ReadConfigFile(ConfigPath(name))
...
function NHUI:GetSavedConfig(name)
	local envelope, err = ReadConfigFile(ConfigPath(name))
	if not envelope then return nil, err end
	return envelope, nil
end
 
function NHUI:ListConfigs()
...
function NHUI:DeleteConfig(name)
...
function NHUI:RenameConfig(oldName, newName)
...
function NHUI:CreateSnapshot()
	return { Data = NHUI:GetConfig(), CreatedAt = os.time() }
end
 
function NHUI:RestoreSnapshot(snapshot, silent)
	if type(snapshot) ~= "table" or type(snapshot.Data) ~= "table" then
		return false, "invalid snapshot"
	end
	return NHUI:SetConfig(snapshot.Data, silent)
end
 
local CLOUD_IDENTITY_PATH = "NHUI/cloud_identity.json"
...
function NHUI:CloudService(opts)
	opts = opts or {}
	...
	local function apiRequest(method, path, body, extraHeaders)
		...
		local headers = {
			["Content-Type"] = "application/json",
			["X-NHUI-Identity"] = identity.Id,
			["X-NHUI-Script"] = scriptId,
		}
	...
	function api:Publish(meta, data)
		...
		local cleanName, nameBlocked = NHUI:SanitizeText(meta.Name, { MaxLength = 60 })
		if nameBlocked or cleanName == "" then
			return nil, "Name was empty or blocked by the content filter."
		end
		local cleanDesc, descBlocked = NHUI:SanitizeText(meta.Description or "", { MaxLength = 280 })
		if descBlocked then
		...
		for _, tag in ipairs(meta.Tags or {}) do
			local cleanTag = NHUI:SanitizeText(tag, { MaxLength = 24 })
...
function NHUI:CreateAIAssistant(opts)
	opts = opts or {}
...
local function DestroyAllWindowJanitors()
	for _, win in ipairs(NHUI._Windows) do
		win._destroyed = true
		if win._janitor then win._janitor:Destroy() end
	end
	table.clear(NHUI._Windows)
end
 
NHUI._Root.Destroying:Connect(function()
...
function NHUI:Unload()
	CloseAnyOpenPopup()
	AcrylicShuttingDown = true
	DestroyAllWindowJanitors()
	DestroyAllAcrylicControllers()
	LibJanitor:Destroy()
	if NHUI._Root then NHUI._Root:Destroy() end
end
 
do
	local globalTable = GetGlobalTable()
	globalTable.__NHUI_Unload = function()
		pcall(function() NHUI:Unload() end)
	end
end
