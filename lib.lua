...
function image.assign(instance: Instance, property: string, value: unknown)
    ...
    if value == "UserAvatar" then
        ...
        if player then
            target[property] = image.avatar(player.UserId, function(uri)
...
function image.resolve(value: unknown): string
    ...
    if value == "UserAvatar" then
        local player = variables.localPlayer
        return if player then image.avatar(player.UserId) else ""
    end
...
function persistenceConfig.list(window: ConfigWindow): { string }
    ...
    for _, filePath in files do
        ...
        if file:sub(-5) == ".rfld" then
            ...
            if base and base ~= "" and base ~= "_default" and not base:find(" %(Incorrect Format[^%)]*%)$") then
                table.insert(names, base)
            end
        elseif file:sub(-5) == ".json" and file ~= "Options.json" then
            local base = path.stripExtension(file, ".json")
            if base and base ~= "" and base ~= "_default" then
                table.insert(names, base)
            end
        end
    end
...
function paths.getConfigPath(window: ConfigWindow, name: unknown?): (string?, string?)
    ...
    local safe = path.sanitizeFile(window.configuration.fileName or "_default")
    if safe == "" then
        safe = "_default"
    end
    return dir, path.join(dir, safe .. ext)
...
function Window.new(properties)
    ...
    self:Create("UIListLayout", {
        FillDirection = Enum.FillDirection.Vertical,
        HorizontalAlignment = Enum.HorizontalAlignment.Center,
        VerticalAlignment = Enum.VerticalAlignment.Top,  -- top, bukan center
        Padding = UDim.new(0, 8),                        -- positif, bukan -10
        SortOrder = Enum.SortOrder.LayoutOrder,
        Parent = self.logoFrame,
    })

    -- logo gambar DULU (LayoutOrder 0 = paling atas)
    self.logoLabel = self:Create("ImageLabel", {
        Image = self.logo,
        Size = UDim2.fromOffset(self.logoSize, self.logoSize),
        BackgroundTransparency = 1,
        ImageTransparency = 1,
        LayoutOrder = 0,        -- <-- ini yang diubah dari 1 ke 0
        Parent = self.logoFrame,
    }, { ImageColor3 = "TitlingColor" })
    ...
    local function applyLogoAspect(a: number?)
        ...
        self.logoFrame.Size = UDim2.new(1, -30, 0, logoH + titleH + 8)  -- +8 = padding
        self.tabList.Position = UDim2.fromOffset(0, self.logoFrame.Size.Y.Offset + 20)
        self.tabList.Size = UDim2.new(1, 0, 1, -(self.logoFrame.Size.Y.Offset + 20))
    end
...
function Window:_syncLiveAnimation()
    if not self.theme.LiveAnimation then
        ...
        self.windowGradient.Rotation = 270
        self.windowGradient.Offset = Vector2.new(0, -0.1)
...
function Window:_persist(control) end
...
function Window:_buildSettingsUI()
    ...
    self.rfSettings:CreateButton({
        name = "Unload Window",
        description = "Fully removes this window and disconnects everything. Cannot be undone.",
        callback = function()
            self:Popup({
                title = "Unload this window?",
                content = "This disconnects everything and destroys the whole UI. You'll need to re-run the script to bring it back.",
                options = {
                    { text = "Cancel" },
                    {
                        text = "Unload",
